...
bool RigidBodyCollection::UpdateRigidBodyData(sRigidBodyData const rb)
{
	for (size_t i = 0; i < mNumRigidBodies; ++i)
    {
		if (mIds[i] == rb.ID)
		{
      		mXYZCoord[i] = std::make_tuple(rb.x, rb.y, rb.z);

      		mXYZWQuats[i] = std::make_tuple(rb.qx, rb.qy, rb.qz, rb.qw);
