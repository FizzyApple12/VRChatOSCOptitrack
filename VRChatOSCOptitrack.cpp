...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    HWND hWnd = CreateWindow("NATNETSAMPLE", "VRChat OptiTrack Interface", WS_OVERLAPPEDWINDOW,
...
void RenderOGLScene()
{
    ...
    // Draw rigid bodies
    float textX = -3200.0f;
    float textY = 2700.0f + (fmod((clock() / CLOCKS_PER_SEC) * 270.0f, 5400.0f));
    GLfloat x, y, z;
    GLfloat rawx, rawy, rawz;
    GLfloat qx, qy, qz, qw;
    Quat q;
    EulerAngles ea;
    int order;
        ...
        */
        ...
        for (int j = 0; j < 8; j++)
        {
            if (activeId == oscIds[j])
            {
                ...
                sprintf(posAddress, "/tracking/trackers/%d/position", j + 1);
                sprintf(rotAddress, "/tracking/trackers/%d/rotation", j + 1);
        ...
        if (showText)
        {
            ...
            glPrinter.Print(textX, textY, "%32s(id: %3d) (X: %6.2f, Y: %6.2f, Z: %6.2f), (P: %3.1f, Y: %3.1f, R: %3.1f)", rigidBodyName.c_str(), activeId, x, y, z, ea.x, ea.y, ea.z);
            textY -= 100.0f;
        }
...
void DataHandler(sFrameOfMocapData* data, void* pUserData)
{
    ...
    {
        for (size_t i = 0; i < data->Skeletons[s].nRigidBodies; ++i)
        {
            if (!rigidBodies.UpdateRigidBodyData(data->Skeletons[s].RigidBodyData[i]))
            {
                rigidBodies.AppendRigidBodyData((data->Skeletons[s].RigidBodyData + i), 1);
            }
        }
    }
